...
-- FUNC QUE VAI INSERIR AUTOMATICAMENTE NA PRODUTO_PRECO_VENDA
...
-- FUNC QUE VAI INSERIR AUTOMATICAMENTE NA CONTROLE_ESTOQUE AO CRIAR UM PRODUTO NOVO
CREATE OR REPLACE FUNCTION insere_controle_estoque() RETURNS TRIGGER AS $$
DECLARE
  empresa_rec RECORD;
BEGIN
  -- Para cada empresa existente, em ordem crescente
  FOR empresa_rec IN (SELECT DISTINCT IDEMPRESA FROM EMPRESA ORDER BY IDEMPRESA)
  LOOP
    -- Insere um registro zerado na tabela CONTROLE_ESTOQUE
    INSERT INTO CONTROLE_ESTOQUE (IDPRODUTO, IDEMPRESA, QUANTIDADE)
    VALUES (NEW.IDPRODUTO, empresa_rec.IDEMPRESA, 0);
  END LOOP;
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;

-- FUNC QUE VAI INSERIR AUTOMATICAMENTE NA CONTROLE_ESTOQUE
CREATE OR REPLACE FUNCTION atualizar_controle_estoque()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.TIPOMOVIMENTO = 'C' OR NEW.TIPOMOVIMENTO = 'DV' THEN
		--CASO o produto foi inserido no banco porém a trigger insere_controle_estoque() não gravou
        PERFORM 1
...
-- FUNC QUE VAI INSERIR AUTOMATICAMENTE NA CONTROLE_ESTOQUE
CREATE OR REPLACE FUNCTION atualizar_controle_estoque_delete()
RETURNS TRIGGER AS $$
BEGIN
    IF OLD.TIPOMOVIMENTO = 'C' OR OLD.TIPOMOVIMENTO = 'DV' THEN
        UPDATE CONTROLE_ESTOQUE
        SET QUANTIDADE = QUANTIDADE - OLD.QUANTIDADE
        WHERE IDPRODUTO = OLD.IDPRODUTO AND IDEMPRESA = OLD.IDEMPRESA;
	ELSEIF OLD.TIPOMOVIMENTO = 'V' OR OLD.TIPOMOVIMENTO = 'DC' THEN
		UPDATE CONTROLE_ESTOQUE
        SET QUANTIDADE = QUANTIDADE + OLD.QUANTIDADE
		 WHERE IDPRODUTO = OLD.IDPRODUTO AND IDEMPRESA = OLD.IDEMPRESA;
    END IF;
...
-- FUNC QUE VAI ATUALIZAR AUTOMATICAMENTE NA CONTROLE_ESTOQUE QUANDO FOR REALIZADO UM UPDATE NA COMPRA_VENDA
CREATE OR REPLACE FUNCTION atualizar_controle_estoque_update()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_atualizar_controle_estoque_delete
AFTER DELETE ON COMPRA_VENDA
FOR EACH ROW
EXECUTE FUNCTION atualizar_controle_estoque_delete();

CREATE TRIGGER trigger_atualizar_controle_estoque_update
AFTER UPDATE ON COMPRA_VENDA
FOR EACH ROW
EXECUTE FUNCTION atualizar_controle_estoque_update();

CREATE TRIGGER trig_insere_controle_estoque
AFTER INSERT ON PRODUTO
FOR EACH ROW
EXECUTE FUNCTION insere_controle_estoque();
